...
export default function EditTournamentPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const effectiveMapPreviewUrl = () => {
    ...
    const query = [formData.location.address, formData.location.city].filter(Boolean).join(', ');
    if (!query) return '';
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-8 pb-12">
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-black text-gray-900 tracking-tight">Edit Tournament</h1>
          <p className="text-gray-500 text-sm font-medium">Update event details and settings</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="bg-white p-1 rounded-3xl shadow-sm border border-gray-100 overflow-hidden">
          {imagePreview ? (
            ...
            <label className="flex flex-col items-center justify-center w-full aspect-[21/9] rounded-[22px] border-4 border-dashed border-gray-100 bg-gray-50/50 hover:bg-gray-100/50 hover:border-[#1abc60]/30 transition-all cursor-pointer group">
              ...
              <span className="text-lg font-bold text-gray-700">Change Tournament Banner</span>
            ...
            <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-2">
                  ...
                  <input type="text" name="matchType" value={formData.matchType} onChange={handleInputChange} placeholder="e.g. T20, 5-a-side" className="w-full px-5 py-3.5 bg-gray-50/50 border border-gray-100 rounded-2xl focus:outline-none focus:border-[#1abc60] transition-all font-bold text-gray-700" />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Tournament Format</label>
                  <input type="text" name="format" value={formData.format} onChange={handleInputChange} placeholder="e.g. League + Knockout" className="w-full px-5 py-3.5 bg-gray-50/50 border border-gray-100 rounded-2xl focus:outline-none focus:border-[#1abc60] transition-all font-bold text-gray-700" />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Team Size</label>
                  <input type="number" name="teamSize" value={formData.teamSize} onChange={handleInputChange} className="w-full px-5 py-3.5 bg-gray-50/50 border border-gray-100 rounded-2xl focus:outline-none font-bold text-gray-700" />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Min Players per Team</label>
                  <input type="number" name="minPlayers" value={formData.minPlayers} onChange={handleInputChange} className="w-full px-5 py-3.5 bg-gray-50/50 border border-gray-100 rounded-2xl focus:outline-none font-bold text-gray-700" />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Max Substitutes</label>
                  <input type="number" name="maxSubstitutes" value={formData.maxSubstitutes} onChange={handleInputChange} className="w-full px-5 py-3.5 bg-gray-50/50 border border-gray-100 rounded-2xl focus:outline-none font-bold text-gray-700" />
                </div>
              </div>
            </div>
            ...
            <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 space-y-6">
              ...
              <div className="space-y-4">
                <div className="flex gap-2">
                  ...
                  />
                  <button type="button" onClick={addRule} className="px-6 bg-[#1abc60] text-white rounded-2xl font-bold hover:bg-[#16a352] transition-all">Add</button>
                </div>

                <div className="space-y-2">
                  {formData.rules.map((rule, idx) => (
                    <div key={idx} className="flex items-center justify-between p-4 bg-gray-50/50 border border-gray-100 rounded-2xl">
                      <div className="flex items-center gap-3">
                        <div className="w-6 h-6 rounded-full bg-[#1abc60] text-white flex items-center justify-center text-[10px] font-black">{idx + 1}</div>
                        <span className="text-sm font-bold text-gray-700">{rule}</span>
                      </div>
                      <button type="button" onClick={() => removeRule(idx)} className="p-1.5 text-gray-300 hover:text-red-500 transition-colors"><X className="w-4 h-4" /></button>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 space-y-6">
              ...
              <div className="space-y-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <input type="text" name="prizes.winner" value={formData.prizes.winner} onChange={handleInputChange} className="w-full bg-transparent border-none focus:ring-0 text-2xl font-black placeholder:text-white/40" />
                </div>
                <div className="p-6 bg-gradient-to-br from-blue-500 to-blue-600 rounded-3xl text-white space-y-2">
                  ...
                  <input type="text" name="prizes.runnerUp" value={formData.prizes.runnerUp} onChange={handleInputChange} className="w-full bg-transparent border-none focus:ring-0 text-2xl font-black placeholder:text-white/40" />
                </div>
                <div className="md:col-span-2 space-y-2">
                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest ml-1">Other Rewards (Optional)</label>
                  <input type="text" name="prizes.others" value={formData.prizes.others} onChange={handleInputChange} className="w-full px-5 py-3.5 bg-gray-50/50 border border-gray-100 rounded-2xl focus:outline-none font-bold text-gray-700" />
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 space-y-6">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                {existingGallery.map((img, idx) => (
                  <div key={`existing-${idx}`} className="relative aspect-square rounded-2xl overflow-hidden group">
                ...
                {galleryImages.map((file, idx) => (
                  <div key={`new-${idx}`} className="relative aspect-square rounded-2xl overflow-hidden group border-2 border-[#1abc60]">
                    ...
                    <div className="absolute bottom-1 left-1 bg-[#1abc60] text-white text-[8px] px-1 rounded font-bold uppercase">New</div>
                  </div>
                ))}
              ...
              <p className="text-[10px] font-bold text-gray-400 uppercase tracking-tighter">
                Manage up to 8 photos for this tournament.
              </p>
            </div>
          </div>
            ...
            <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 space-y-6">
              <div className="space-y-2">
                ...
                <div className="relative">
                  ...
                  <input type="number" name="entryFee" value={formData.entryFee} onChange={handleInputChange} className="w-full pl-12 pr-5 py-4 bg-green-50/50 border-2 border-transparent focus:border-[#1abc60] rounded-2xl focus:outline-none font-black text-2xl text-[#1abc60]" />
                </div>
              </div>
            </div>
            ...
            <div className="bg-white p-8 rounded-3xl shadow-sm border border-gray-100 space-y-6">
              ...
              <div className="space-y-4">
                ...
                <div className="space-y-2">
                  ...
                  <input required type="date" name="registrationDeadline" value={formData.registrationDeadline} onChange={handleInputChange} className="w-full px-5 py-3 bg-red-50 border border-red-100 rounded-xl focus:outline-none font-bold text-red-600 text-sm" />
                </div>
              </div>
            </div>
        ...
        <div className="bg-white p-8 rounded-3xl shadow-lg border border-gray-100 flex items-center justify-between gap-6">
          <div className="hidden md:block">
            <h3 className="text-lg font-black text-gray-800 tracking-tight">Finish updating?</h3>
            <p className="text-gray-500 text-sm font-medium">Any changes will be updated immediately.</p>
          </div>
