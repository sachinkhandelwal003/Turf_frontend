  ...
  return (
    <div className="fixed bottom-5 right-5 z-[9999]">
      {!isOpen ? (
        <button
          onClick={handleOpen}
          className="bg-gradient-to-br from-[#1abc60] to-[#16a085] text-white p-4 rounded-full shadow-2xl hover:shadow-[0_0_20px_rgba(26,188,96,0.4)] transition-all duration-300 flex items-center justify-center hover:scale-110 active:scale-95"
        >
          ...
          <div className="flex-1 flex flex-col overflow-hidden bg-gradient-to-b from-gray-50 to-white">
            {!user ? (
              <div className="flex-1 flex flex-col items-center justify-center p-8 text-center space-y-5">
                ...
                <Link
                  href="/login"
                  className="w-full py-3 bg-gradient-to-r from-[#1abc60] to-[#16a085] text-white rounded-xl font-bold hover:shadow-lg transition-all no-underline inline-block text-sm"
                  onClick={() => setIsOpen(false)}
                >
                  Log In Now
                </Link>
